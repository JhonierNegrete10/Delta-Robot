...
def c_inversa(L_1: int, L_2: int, D: int, d: int, h: int, P) -> tuple[int, list]:
    ...
    Returns:
        tuple[int, list]: si el int es 0 no existe el punto dentro del espacio
        de trabajo, y la lista son los valores de angulos de los tres motores
        en grados.
    ...
    for idx, i in enumerate(phi):
        ...
        try:
            th2.append(
                rad2deg(
                    acos(
                        (sum(C[idx] ** 2) - L_1**2 - L_2**2)
                        / (2 * L_1 * L_2 * sin(deg2rad(th3[idx])))
                    )
                )
            )

            th1.append(
                rad2deg(atan2(C[idx][2], C[idx][0]))
                - rad2deg(
                    acos(
                        (
                            (L_1**2)
