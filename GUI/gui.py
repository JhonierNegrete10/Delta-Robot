...
class Application(ttk.Frame):
    def __init__(self, master=None):
        ...
        try:
            ...
            self.frame_title = tk.Frame(
                master=self.master, width=800, height=50, relief=tk.SOLID, borderwidth=2
            )
            self.frame_title_in()
            self.frame_title.pack(fill=tk.X)

            self.frame_Body = tk.Frame(master=self.master, width=800, height=350)
            self.frame_body_in()
    ...
    def frame_btton_test_widget(self):
        self.frame_test = tk.Frame(master=self.frame_right, width=400, height=100)
        self.frame_test.pack(fill=tk.BOTH, expand="yes")

        text_test = Label(
            self.frame_test, text="Test cuarto de milla", bd=4, font=("curier 10 bold")
        )

        text_test.pack(side=TOP, fill=tk.X)
        self.frame_btton_test = tk.Frame(master=self.frame_test, width=400, height=50)

        ttk.Button(self.frame_btton_test, text="test", command=self.bttn_test).pack(
            side=TOP
        )
    ...
    def frame_btton_config_widget(self):
        self.frame_config = tk.Frame(master=self.frame_right, width=400, height=100)
        ...
        self.frame_btton_config = tk.Frame(
            master=self.frame_config, width=400, height=50
        )

        ttk.Button(
            self.frame_btton_config, text="Config", command=self.bttn_config
        ).pack(side=TOP)
    ...
    def bttn_config(self):
        ...
        self.frame_btton_save = tk.Frame(master=self.newWindow, width=300, height=50)
        ttk.Button(self.frame_btton_save, text="Save", command=self.bttn_save).pack(
            side=TOP
        )
    ...
    def entrys(self, Window):
        ...
        self.list_frames_nw = [
            tk.Frame(master=Window, width=300, height=50)
            for i in self.delta_config.keys()
        ]
        id = 0
        for key, value in self.delta_config.items():
            self.labels_config[key] = Label(
                self.list_frames_nw[id], text=key, bd=0, font=("curier 11 bold")
            )
            ...
            self.entrys_config[key] = Entry(
                self.list_frames_nw[id], textvariable=self.config_StringVar[key]
            )
    ...
    def frame_btton_home_widget(self):
        self.frame_home = tk.Frame(master=self.frame_right, width=400, height=100)
        ...
        self.frame_btton_home = tk.Frame(master=self.frame_home, width=400, height=50)

        ttk.Button(self.frame_btton_home, text="Home", command=self.bttn_home).pack(
            side=TOP
        )
    ...
    def frame_btton_enviar_widget(self):
        self.frame_enviar = tk.Frame(master=self.frame_right, width=400, height=50)
        ...
        self.frame_btton_enviar = tk.Frame(
            master=self.frame_enviar, width=400, height=50
        )

        ttk.Button(
            self.frame_btton_enviar, text="Enviar", command=self.bttn_enviar
        ).pack(side=TOP)
    ...
    def transform(self):
        ...
        for i, value in enumerate(self.puntos["x"]):
            Punto = np.array(
                [int(value), int(self.puntos["y"][i]), int(self.puntos["z"][i])]
            )
            k, movimientos = c_inversa(
                L_1=self.delta_config["L_1"],
            ...
            if k:
                ...
                messagebox.showerror(
                    "Error", f"El punto {Punto} no existe dentro del espacio de trabajo"
                )
        if list_t1:
            payload = {
                "Micros": 2 ** int(self.micro.get()),
    ...
    def frame_ip_widget(self):
        self.frame_ip = tk.Frame(master=self.frame_left, width=400, height=50)
    ...
    def frame_micro_widget(self):
        self.frame_micro = tk.Frame(master=self.frame_left, width=400, height=150)

        text_micro = Label(
            self.frame_micro, text="Microstepping", bd=4, font=("curier 14 bold")
        )
        text_micro.pack()

        self.text_sel_micro = Label(
            self.frame_micro, text="Seleciona un Microstepping", bd=4, font=("curier 9")
        )
        ...
        for id, value in enumerate(self.list_micros):
            self.dict_micros[value] = Radiobutton(
                self.frame_micro,
                text=value,
                value=id,
                variable=self.micro,
    ...
    def sel(self):
        selection = "Has seleccionado " + str(self.list_micros[self.micro.get()])
    ...
    def frame_puntos_widget(self):
        ...
        self.list_frames = [
            tk.Frame(master=self.frame_puntos, width=400, height=100)
            for i in self.list_ejes
        ]

        for id, value in enumerate(self.list_ejes):
            self.dict_ejes[value] = Label(
                self.list_frames[id], text=value, bd=0, font=("curier 11")
            )
            ...
            self.dict_ejes[value] = Entry(
                self.list_frames[id], textvariable=self.ejes_StringVar[value]
            )
    ...
    def frame_btton_agregar_widget(self):
        self.list_text = [
            tk.Frame(master=self.frame_left, width=400, height=100)
            for i in self.list_ejes
        ]
        self.dict_texts = {}
        for id, value in enumerate(self.list_ejes):
            self.dict_texts[value] = Label(
                self.list_text[id], text=value, bd=0, font=("curier 11")
            )

            self.dict_texts[value].pack(side=LEFT)

        ttk.Button(self.frame_left, text="Agregar", command=self.bttn_agregar).pack()
